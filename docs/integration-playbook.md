...
> 冻结区说明（P01 §11/§13）：本文件登记**必须等待宿主集成期才能闭环**的 seam。
...
| 6 | 四个白名单接线点（P01 §5.1）：`tsconfig.host.json` +1 行、`tsconfig.client.json` +1 行、web-app `cordis.patch.yml` insert 行、`pnpm-lock.yaml` workspace importer | 分支集成窗口开启（用户明确指令，AutoPilot 战役接线点串行合并之后） | 按 P01 §10 checklist 执行四文件改动；bundle 行 id 固定 `filehub` | 分支部署全新 profile：上传/@ 提及/AI 读文档/控制台四链路免配置可用；diff-baseline 门绿 |

## 二、集成期红线

1. **权威源唯一**：集成以本仓库为事实来源，禁止在 Fork 里手改后反向拷贝（Vision 双仓发散教训，P01 §11）。
2. **接线点 ≤4**：除上表第 5 行四个白名单文件外，任何 upstream 文件零改动；CI diff-baseline 门持续把关。
3. **Typert 两铁律**（继承 R2/R5 结论）：远程服务手写 `ctx.typert.register` manifest；客户端用 `ctx.reflect.get('remote.<ns>')` 解析。M7 演练时若启用 Typert 面，按此执行。
4. **净室红线**：PluginR&D/sources 社区仓代码永不进入构建树；依赖黑名单守卫持续运行。
